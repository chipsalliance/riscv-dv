...
def process_jal(trace, operands, gpr):
    ...
    if len(operands) == 2:
        ...
        fatal("process_jal(%s) wrong num operands (%d)" % (trace.instr, len(operands)))
...
def show_line_instr(line, i):
    if i.instr_str[0] in ['v']:
        ...
        logging.debug("  -->> instr_str(%s) binary(%s) addr(%s) mode(%s) instr(%s)" 
            % (      i.instr_str, i.binary,  i.addr, i.privileged_mode, i.instr))

def check_num_operands(instr_str, num_operands, n):
    if n != num_operands:
        fatal("%s: num operands wrong, expected (%d) got (%d)" % (instr_str, n, num_operands))

def is_csr(r):
    if r in ["mtvec","pmpaddr0","pmpcfg0","mstatus","mepc","mscratch","mcause","mtval","vl","vtype"]:
        return True
    else:
        return False

# ovpsim logs branch offsets as absolute
...
gpr = {}
csr = {}
def process_ovpsim_sim_log(ovpsim_log, csv, full_trace = 1, stop = 1, dont_truncate_after_first_ecall = 0):
  ...
  logging.info("Processing ovpsim log [%s %s %s]: %s" % 
    ...
    "dont_truncate_after_fist_ecall" if dont_truncate_after_first_ecall else "", ovpsim_log))
  instr_cnt = 0
    ...
    for line in f:
      ...
      if m:
        ...
        if full_trace:
            ...
            o = re.search (r"(?P<instr>[a-z]*?)\s(?P<operand>.*)", prev_trace.instr_str)
            if o:
                ...
                if (prev_trace.instr in ['jalr', 'c.jalr']):
                  ...
                  if 'v' in prev_trace.instr[0]:
                    ...
                    elif prev_trace.instr in ['c.beqz', 'c.bnez', 'beqz', 'bnez', 'bgez', 'bltz', 'blez', 'bgtz']:
        ...
        if c and is_csr (c.group("r")):
            ...
            if verbose2: logging.debug("c:csr %0s = %0s" % (c.group("r"), c.group("val")))
            csr[c.group("r")] = c.group("val")  
        ...
        n = re.search(r" (?P<r>[a-z]{1,3}[0-9]{0,2}?) (?P<pre>[a-f0-9]+?)" \
                       " -> (?P<val>[a-f0-9]+?)$", line)
        if n: # gpr
          if verbose2: logging.debug(("n:gpr %0s = %0s" % (n.group("r"), n.group("val"))))
          if n.group("r") != "frm":
            ...
            line = line.strip()
            if verbose2: logging.debug("ignoring line: [%s] %s " % (str(instr_cnt), line))
            line = re.sub(' +', ' ', line)
            split = line.split(" ")
            if len(split) == 1: continue
            item = split[1]
            if "----" in item: continue
            if "REPORT" in line or item in ["mtvec","pmpaddr0","pmpcfg0","mstatus","mepc","mscratch","mcause","mtval","vl","vtype"]:
                pass
            else:
                fatal ("<unknown> (%s) in line: [%s] %s " % (item, str(instr_cnt), line))
...
def main():
  ...
  parser.add_argument("--dont_truncate_after_first_ecall", dest="dont_truncate_after_first_ecall", action="store_true",
  ...
  # Process ovpsim log
  process_ovpsim_sim_log(args.log, args.csv, args.full_trace, args.stop_on_first_error, args.dont_truncate_after_first_ecall)
