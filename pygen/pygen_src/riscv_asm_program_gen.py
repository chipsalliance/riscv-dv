...
class riscv_asm_program_gen:
    ...
    def gen_program(self):
        ...
        for hart in range(cfg.num_of_harts):
            ...
            self.generate_directed_instr_stream(hart=hart,
                                                label=self.main_program[
                                                    hart].label_name,
                                                original_instr_cnt=
                                                self.main_program[
                                                    hart].instr_cnt,
                                                min_insert_cnt=1,
                                                instr_stream=self.main_program[
                                                    hart].directed_instr)
            ...
            for hart in range(cfg.num_of_harts):
                ...
                if not cfg.no_data_page:
                    # self.gen_data_page() # Unimplemented
    ...
    def gen_kernel_sections(self, hart):
        ...
        self.instr_stream.append(
            pkg_ins.get_label("kernel_instr_start:", hart))
    ...
    def gen_program_header(self):
        ...
        for hart in range(cfg.num_of_harts):
            string.append(
                "li x6, {}\n{}beq x5, x6, {}f".format(hart, pkg_ins.indent,
                                                      hart))
    ...
    def gen_stack_section(self, hart):
        ...
        if cfg.use_push_data_section:
            self.instr_stream.append(
                ".pushsection .{}user_stack,\"aw\",@progbits;".format(
                    hart_prefix_string))
        else:
            self.instr_stream.append(
                ".section .{}user_stack,\"aw\",@progbits;".format(
                    hart_prefix_string))
        if rcs.SATP_MODE != "BARE":
            self.instr_stream.append(".align 12")
        else:
    ...
    def gen_kernel_stack_section(self, hart):
        ...
        if cfg.use_push_data_section:
            self.instr_stream.append(
                ".pushsection .{}kernel_stack,\"aw\",@progbits;".format(
                    hart_prefix_string))
        else:
            self.instr_stream.append(
                ".section .{}kernel_stack,\"aw\",@progbits;".format(
                    hart_prefix_string))
        if rcs.SATP_MODE != "BARE":
            self.instr_stream.append(".align 12")
        else:
        ...
        self.instr_stream.append(
            pkg_ins.get_label("kernel_stack_start:", hart))
        self.instr_stream.append(".rept {}".format(cfg.kernel_stack_len - 1))
        self.instr_stream.append(".{}byte 0x0".format(rcs.XLEN // 8))
        self.instr_stream.append(".endr")
    ...
    def setup_misa(self):
        ...
        misa = 0b01000000
        self.instr_stream.append(
            "{}li x{}, {}".format(pkg_ins.indent, cfg.gpr[0].value, hex(misa)))
        self.instr_stream.append(
            "{}csrw misa, x{}".format(pkg_ins.indent, cfg.gpr[0].value))
    ...
    def init_gpr(self):
        ...
        for i in range(32):
            init_string = "{}li x{}, {}".format(pkg_ins.indent, i,
                                                random.choice(dist_lst))
        ...
        for i in range(32):
            if rcs.XLEN == 64:
                ...
                string = "{}sw x{}, {}(x{})".format(
                    pkg_ins.indent, i, int(i * (rcs.XLEN / 8)),
                    cfg.gpr[0].value)
            self.instr_stream.append(string)
    ...
    def pre_enter_privileged_mode(self, hart):
        ...
        string.append("la x{}, {}kernel_stack_end".format(cfg.tp.value,
                                                          pkg_ins.hart_prefix(
                                                              hart)))
        self.gen_section(pkg_ins.get_label("kernel_sp", hart), string)

        if (not cfg.no_delegation and (
    ...
    def setup_epc(self, hart):
        instr = []
        instr.append("la x{}, {}init".format(cfg.gpr[0].value,
        ...
        if (not rcs.support_pmp):  # TODO
            instr.append("j {}init_{}".format(pkg_ins.hart_prefix(hart),
    ...
    def trap_vector_init(self, hart):
        ...
        for items in rcs.supported_privileged_mode:
            if items == "MACHINE_MODE":
                ...
                logging.critical(
                    "[riscv_asm_program_gen] Unsupported privileged_mode {}".format(
                        items))
    ...
    def gen_interrupt_handler_section(self, mode, hart):
        ...
        # return
        if (mode is privileged_mode_t.USER_MODE.name and not (
                rcs.support_umode_trap)):
            return
        ...
        if cfg.enable_nested_interrupt:
            interrupt_handler_instr.append(
                "csrr x%0d, 0x%0x" % (cfg.gpr[0].value, scratch.value))
            interrupt_handler_instr.append(
                "bgtz x%0d, 1f" % (cfg.gpr[0].value))
            interrupt_handler_instr.append(
                "csrwi 0x%0x, 0x1" % (scratch.value))

            if (status == privileged_reg_t.MSTATUS):
                interrupt_handler_instr.append(
                    "csrsi 0x%0x, 0x%0x" % (status.value, 8))
            elif (status == privileged_reg_t.SSTATUS):
                interrupt_handler_instr.append(
                    "csrsi 0x%0x, 0x%0x" % (status.value, 2))
            elif (status == privileged_reg_t.USTATUS):
                interrupt_handler_instr.append(
                    "csrsi 0x%0x, 0x%0x" % (status.value, 1))
            else:
                logging.critical("Unsupported status %0s" % (status.value))

            interrupt_handler_instr.append(
                "1: csrwi 0x%0x,0" % (scratch.value))
        ...
        pkg_ins.pop_gpr_from_kernel_stack(status.name, scratch.name,
                                          cfg.mstatus_mprv,
                                          cfg.sp, cfg.tp,
                                          interrupt_handler_instr)
    ...
    def gen_section(self, label, instr):
        if label != "":
            string = pkg_ins.format_string("{}:".format(label),
    ...
    def gen_signature_handshake(self, instr, signature_type,
                                ...
                                test_result="TEST_FAIL", csr="MSCRATCH",
                                addr_label=""):
        pass

    def add_directed_instr_stream(self, name, ratio):
        self.directed_instr_stream_ratio[name] = ratio
        logging.info("Adding directed instruction stream:%0s ratio:%0d/1000",
                     name, ratio)
    ...
    def get_directed_instr_stream(self):
        ...
        for i in range(cfg.max_directed_instr_stream_seq):
            ...
            if arg in args:
                ...
                if len(opts) != 2:
                    logging.critical(
                        "Incorrect directed instruction format : %0s, expect: name,ratio",
                        val)
                else:
    ...
    def generate_directed_instr_stream(self, hart=0, label="",
                                       original_instr_cnt=0,
                                       min_insert_cnt=0, kernel_mode=0,
                                       instr_stream=[]):
        instr_insert_cnt = 0
        idx = 0
