...
class SeedGen:
    ...
    def __init__(self, start_seed, fixed_seed, seed_yaml):
        ...
        # parsing time, but it can't hurt to do a belt-and-braces check here
        # too.
    ...
    def get(self, test_id, test_iter):
        ...
        if test_id in self.rerun_seed:
            # Note that test_id includes the iteration index (well,
            # the batch index, at any rate), so this makes sense even if
            # test_iter > 0.
...
def get_generator_cmd(simulator, simulator_yaml, cov, exp, debug_cmd):
    """ Setup the compile and simulation command for the generator
      ...
      Returns:
        compile_cmd    : RTL/pyflow simulator command to compile the
    ...
    logging.error("Cannot find RTL/pyflow simulator %0s" % simulator)
    sys.exit(RET_FAIL)
...
def do_simulate(sim_cmd, simulator, test_list, cwd, sim_opts, seed_gen,
  ...
  Args:
    ...
    end_signature_addr    : Address that tests will write pass/fail signature
    to at end of test
    lsf_cmd               : LSF command used to run the instruction generator
    ...
    for test in test_list:
        ...
        if iterations > 0:
            ...
            if test['test'] == 'riscv_csr_test':
                ...
                logging.info(
                    "Running %s with %0d batches" % (test['test'], batch_cnt))
                for i in range(0, batch_cnt):
                    ...
                    if simulator == "pyflow":
                        cmd = lsf_cmd + " " + sim_cmd.rstrip() + \
                              (" {}/pygen/pygen_src/test/{}.py "
                               .format(cwd, test['gen_test'])) + \
                    ...
                    if lsf_cmd:
                        ...
                        logging.info("sim command: %s" % cmd)
                        run_cmd(cmd, timeout_s,
                                check_return_code=check_return_code,
                                debug_cmd=debug_cmd)
    if sim_seed:
        with open(('%s/seed.yaml' % os.path.abspath(output_dir)),
                  'w') as outfile:
...
def gen(test_list, argv, output_dir, cwd):
    ...
    if not argv.so and argv.simulator != "pyflow":
    ...
    if not argv.co:
        ...
        do_simulate(sim_cmd, argv.simulator, test_list, cwd, argv.sim_opts,
                    seed_gen,
                    argv.csr_yaml,
                    argv.isa, argv.end_signature_addr, argv.lsf_cmd,
                    argv.gen_timeout, argv.log_suffix,
                    argv.batch_size, output_dir, argv.verbose,
                    check_return_code, argv.debug)
  ...
  """
    ...
    for test in test_list:
        for i in range(0, test['iterations']):
            ...
            for iss in iss_list:
                log_list.append(
                    "%s/%s_sim/%s.%d.log" % (output_dir, iss, test['test'], i))
...
def parse_args(cwd):
    ...
    parser.add_argument("-i", "--iterations", type=int, default=0,
                        ...
                        dest="iterations")
    # SVGen or PyGen
    # We may need to think about a replacement for pygen as per Scott's
    # suggestion, I'm using pyflow for now
    parser.add_argument("--pyflow", dest="pyflow", action="store_true",
                        default=False,
                        help="Enable PyGen flow for instruction generation "
    ...
    if not args.custom_target:
        ...
        if re.match(".*gcc_compile.*", args.steps) or re.match(".*iss_sim.*",
            ...
            if (not args.mabi) or (not args.isa):
                sys.exit(
                    "mabi and isa must be specified for custom target %0s" % args.custom_target)
        if not args.testlist:
    ...
    try:
        ...
        if not args.co:
            ...
            for t in list(matched_list):
                # Check mutual exclusive between gen_test, asm_tests,
                # and c_tests
                if 'asm_tests' in t:
        ...
        if args.steps == "all" or re.match(".*gen.*", args.steps):
            # Run any handcoded/directed assembly tests specified in YAML
            # format
                ...
                for test_entry in asm_directed_list:
                    ...
                    if path_asm_test:
                        ...
                        # path_asm_test is an assembly file
                            ...
                            run_assembly(path_asm_test, args.iss_yaml,
                                         args.isa, args.mabi, gcc_opts,
                                         args.iss, output_dir,
                                         args.core_setting_dir, args.debug)
                        else:
                            if not args.debug:
                                logging.error(
                                    '%s does not exist' % path_asm_test)
                                sys.exit(RET_FAIL)
                ...
                for test_entry in c_directed_list:
                    ...
                    if path_c_test:
                        ...
                        if os.path.isdir(path_c_test):
                            run_c_from_dir(path_c_test, args.iss_yaml,
                                           args.isa, args.mabi,
