...
`define INSTR_BODY(instr_n, instr_format, instr_category, instr_group, imm_tp = IMM) \
 ...
 `define V_INSTR_BODY(instr_n, instr_format, instr_category, instr_group, vav, imm_tp) \
    static bit valid = riscv_instr::register(instr_n);  \
    `uvm_object_utils(riscv_``instr_n``_instr)  \
    function new(string name = "");  \
      super.new(name);  \
...
// Vector instruction
`define DEFINE_V_INSTR(instr_n, instr_format, instr_category, instr_group, vav = {}, imm_tp = IMM)\
  class riscv_``instr_n``_instr extends riscv_vector_instr;  \
    `V_INSTR_BODY(instr_n, instr_format, instr_category, instr_group, vav, imm_tp)
